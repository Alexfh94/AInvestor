...
PROFILE_PROMPT_INSTRUCTIONS = {
    "extreme": (
        ...
        "Leverage 20x always. "
        "Take-profit MUST be 1.2% on price (= 24% ROE at 20x; mandatory ROE take-profit at +6% ROE ≈ 0.3% price). "
        "Stop-loss is AUTOMATIC at -6% ROE (0.3% price at 20x), monitored every 1 second when a position is open — "
        "losses never run past -6% ROE, so pick entries worth that risk. "
        ...
        "Do NOT re-open a symbol+side that just closed (cooldown 1 cycle ≈5 min; longer after a loss). "
        "HOLDING RULE (hard-enforced): you CANNOT close a position younger than 1h while its ROE is "
        "between -3% and +5% — let winners run to the trailing stop (+4% ROE) or TP (+6% ROE). "
        "Closing a winner early to rotate costs ~2% fees plus the winner's remaining upside. "
        "When IN LOSS between -3% and -6% ROE: HOLD only if quant + trend still support your direction; "
        "otherwise CLOSE at 100% — do not hope without signal backing. "
        "System auto-closes at +6% ROE (take profit) and at -6% ROE (stop) regardless of signals. "
