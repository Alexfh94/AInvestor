...
def test_approve_valid_buy(db_session):
    ...
    result = risk.validate_proposal(proposal, _snapshot(), current_price=50000.0)
...
def test_reject_non_whitelist_symbol(db_session):
    ...
    result = risk.validate_proposal(proposal, _snapshot(), current_price=0.00001)
...
def test_reject_kill_switch_active(db_session):
    ...
    result = risk.validate_proposal(
        proposal, _snapshot(kill_switch_active=True), current_price=3000.0
    )
    assert result.approved is False
